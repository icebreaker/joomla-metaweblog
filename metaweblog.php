...
class plgXMLRPCmetaWeblogServices
{
	...
	function newPost($blogid, $username, $password, $content, $publish)
	{
		...
		// we should rewrite this to take advantage of the Article `model` which
		// triggers the `hooks` and does a lot of hard-work but for now it is just
		// easier and more convenient to monkey-patch this and call the before save
		// hook ourselves right here
	...
	function getPost($postid, $username, $password)
	{
		...
		{
			return new xmlrpcresp(0, $xmlrpcerruser+2, print_r($category,true));
	...
	function newMediaObject($blogid, $username, $password, $file)
	{
		...
		if (!$ftp['enabled'] && JPath::isOwner($file) && (!JPath::setPermissions($dir, '0555') || !JPath::setPermissions($tmp_dir, '0555'))) {
